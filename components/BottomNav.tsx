...
export default function BottomNav() {
  ...
  const Item = ({ href, icon: Icon, label, exact = false }: { href: string; icon: any; label: string; exact?: boolean }) => (
    <Link
      href={href}
    ...
    <nav className="fixed bottom-0 left-0 right-0 z-30 bg-white/95 dark:bg-black/95 border-t border-slate-200 dark:border-[#1f1f1f] backdrop-blur supports-[backdrop-filter]:bg-white/80 supports-[backdrop-filter]:dark:bg-black/80">
      <div className="mx-auto max-w-3xl">
        <div className="grid grid-cols-5 gap-1 p-2">
          ...
          <Item href="/dashboard/new" icon={PlusSquare} label="Buat" />
          <Item href="/dashboard/roadmaps" icon={ListChecks} label="Saya" />
          <Item href="/dashboard/profile" icon={User} label="Profil" />
        </div>
      </div>
    </nav>
  );
