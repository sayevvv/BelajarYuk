...
export default function RoadmapTracker({ roadmapId }: { roadmapId: string }) {
  ...
  return (
    <div className="h-full overflow-hidden bg-white flex flex-col">
      {/* Header */}
      <header className="flex items-center justify-between gap-4 p-6 border-b border-slate-200 sticky top-0 bg-white/80 backdrop-blur-sm z-10">
        <div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <ol className="space-y-4">
          {milestones.map((m, mi) => (
            <li key={mi} className="rounded-xl border border-slate-200 bg-white">
                  ...
                  {m.sub_tasks.map((task, ti) => {
                    ...
                    return (
                      <li key={ti} className="flex items-center gap-3 px-5 py-3">
                        ...
                        />
                        <label htmlFor={`${key}`} className={`text-slate-800 ${done ? 'line-through text-slate-400' : ''}`}>{task}</label>
                      </li>
                    );
                  })}
                </ul>
